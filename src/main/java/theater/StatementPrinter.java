...
public class StatementPrinter {
    ...
    private static final int TRAGEDY_EXTRA_AMOUNT = 1000;
    private static final int TRAGEDY_AUDIENCE_THRESHOLD = 30;
    private static final int AMOUNT_DIVISOR = 100;
    ...
    private static String usd(int totalAmount) {
        return NumberFormat.getCurrencyInstance(Locale.US).format(totalAmount / AMOUNT_DIVISOR);
    }
    ...
    private int getAmount(Performance performance) {
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                rslt = TRAGEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    rslt += TRAGEDY_EXTRA_AMOUNT * (performance.getAudience() - TRAGEDY_AUDIENCE_THRESHOLD);
                }
                break;
            case "comedy":
